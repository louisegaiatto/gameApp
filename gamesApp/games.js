...
function listaJogos(lisJogos){
	...
	function inicializaSelecaoRemetentes(lista){  
		inicializaSelecao(lista, "forum_remetente");
	}
	function insereForum(){	
		var titulo = document.getElementById('forum_titulo').value;
		var mensagem = document.getElementById('forum_mensagem').value;
		var remetentes = document.getElementById('forum_remetente');		
		var remetente = remetentes.options[remetentes.selectedIndex].innerText;
		parms= "&titulo="+ titulo +"&mensagem="+ mensagem +"&remetente="+ remetente;
		ajaxCall("games.php?action=insForum" + parms, listaForum);
	}
